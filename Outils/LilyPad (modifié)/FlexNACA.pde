...
class FlexNACA extends NACA{
  ...
  FlexNACA( float x, float y, float c, float t, float vp, float _k, float[] _a, Window window ){
...
// set the wave parameters
    x0 = x-0.25*c;
    k = _k*TWO_PI/c;
    omega = vp*k;//vp vitesse de propagation de l'onde
    T = TWO_PI/omega; //période

// set the amplitude envelope    
    a = _a;
    float s = 0; 
    for(float ai: a) s += ai;
    if(s==0) {s=1; a[0]=1;} //met un tube enveloppe par défaut
    //for(int i=0; i<a.length; i++) a[i] *= 1/s;//norme les coefficients de l'enveloppe
...
/* The wave is given by h = A(x)*sin(k*x-omega*t)
  ...
  //float arg( float x)   { return -omega*time; }
  //float arg( float x)   { if (x<c/2){
                            //return -omega*time;} 
                          // else{
                           // return -omega*time-3.14;}
  //}
